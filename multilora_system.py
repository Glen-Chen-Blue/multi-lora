...
class MultiLoRAEngine:
    def __init__(self, model_id: str, r: int = 16, alpha: int = 64, device: Optional[str] = None, torch_dtype: torch.dtype = torch.bfloat16, enable_monitor: bool = True, adapter_fetcher: Optional[Callable[[str], bytes]] = None):
        ...
        # [Capacity Configuration] 讀取 config.py
        ...
        # [核心修正] 分離鎖定層級，避免阻塞 metrics API
        self.lock = threading.RLock()       # 輕量級隊列狀態鎖
        self.gpu_lock = threading.Lock()    # 重型 GPU 資源與推論鎖
    ...
    def _ensure_cpu_loaded(self, adapter_id: str):
        ...
        # Phase 2: 無鎖下載與解析 (Disk I/O)，不卡死 API
        try:
    ...
    def _cleanup_unused_adapters(self):
        ...
        # 保護 Merged Adapter 以備 Unmerge 需要
        if self.current_merged_adapter:
    ...
    def step(self) -> bool:
        ...
        with self.lock:
            ...
            if self.current_merged_adapter:
                ...
                for i, req in enumerate(self.request_queue):
                    ...
                    # 核心限制：確保新進 Request 的 Batch 開銷加上 LoRA 開銷不會超過 unmerged_capacity
                    ...
                    if total_cost <= self.unmerged_capacity:
                        ...
                        break # 容量已滿，停止把 Request 塞進這個 Batch
        ...
        # ==========================================
        # Phase 2: 無鎖 I/O 載入 CPU，不卡 Metrics
        # ==========================================
        for aid in required_adapters:
            self._ensure_cpu_loaded(aid)

        # ==========================================
        # Phase 3: 上鎖整理張量，準備送入 GPU
        # ==========================================
        with self.lock:
            ...
            input_ids, attention_mask, batched_past, mapping = None, None, None, None
            is_prefill = False
            batch_reqs = []
        ...
        # ==========================================
        # Phase 4: 上 GPU 鎖運算，解開隊列鎖 (Metrics順暢)
        # ==========================================
        try:
            with self.gpu_lock:
            ...
            if "out of memory" in str(e).lower():
                ...
                with self.lock:
                    if self.running_queue: self.running_queue.pop()
                torch.cuda.empty_cache()
                return True
            else:
                raise e
