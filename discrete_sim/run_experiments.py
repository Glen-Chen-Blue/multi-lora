...
def run_plot(args):
    ...
    label_map = {
        ...
        5: "Experiment 5 (SLora)",  # 🔄 將原本的 Experiment 4 改名為 Experiment 6
        # ✂️ 移除了原本的 Experiment 6 (Slora)
    }

    # ✂️ 將 range(1, 7) 改為 range(1, 6)，這樣迴圈只會抓取 1 到 5 的 log，直接跳過本來的 6
    for i in range(1, 6):
        log_path = os.path.join(args.output_dir, f"experiment_single_cluster_2nodes{i}_logs", "efo_global_metrics.log")
        if os.path.exists(log_path):
            log_files.append(log_path)
...
def main():
    ...
    if args.plot:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(run_single, args, i) for i in range(1, 6)]
