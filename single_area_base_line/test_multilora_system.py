...
# ============================================================
# Dynamic LoRA Layer (Simplified: No Merge Logic)
# ============================================================
class DynamicLoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adapter_mapping: Optional[torch.Tensor] = None) -> torch.Tensor:
...
# ============================================================
# Multi-LoRA Engine Core (Fixed Batch, No Merge)
# ============================================================
class MultiLoRAEngine:
    def __init__(self, model_id: str, r: int = 16, alpha: int = 64, adapter_slots: int = 8, max_batch_size: int = 4, device: Optional[str] = None, torch_dtype: torch.dtype = torch.bfloat16):
        self.device = torch.device(device) if device else torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.dtype = torch_dtype if self.device.type == "cuda" else torch.float32
        
        # [Comparison] 固定 Batch Size
        self.max_batch_size = int(max_batch_size)
        self.adapter_slots = int(adapter_slots)
        
        print(f"⏳ [BaselineEngine] Loading base model: {model_id} on {self.device}...")
    ...
    def load_adapters_to_cpu(self, base_path: str = "../testLoRA"):
        ...
        for d in dirs:
            ...
            try:
                ...
                if adapter_weights: 
                    self.cpu_cache[aid] = adapter_weights
            except Exception as e:
                print(f"❌ Failed to load {aid}: {e}")
        print(f"✅ [BaselineEngine] All adapters loaded to CPU. Count: {len(self.cpu_cache)}")
    ...
    def step(self) -> bool:
        with self.lock:
            ...
            # 2. 單純的調度邏輯：Slot 夠用就放進來，直到 Max Batch Size
            while len(self.running_queue) < self.max_batch_size and self.request_queue:
                ...
                # 若該請求的 Adapter 還沒載入，且 Slot 已滿 (且沒地方可換)，就不能進
                # 這裡簡化：只要 Slot 總數夠，我們假設 _ensure_adapters_resident 會處理置換
                # 但要防止同時 Running 的 Adapter 超過 Slot 數
                unique_running = current_aids | {req["adapter_id"]}
                if len(unique_running) > self.adapter_slots:
                    break 
    ...
    def _apply_mapping(self, mapping):
        # 為了簡化，直接將 mapping 傳遞給所有 DynamicLoRALinear
        # 這邊用一種比較 hack 的方式，或者修改 forward
        # 我們在 DynamicLoRALinear 的 forward 加了 adapter_mapping 參數，但 model() 呼叫不會傳
        # 所以還是用 Context Variable 比較好，這裡模擬 Context
        # 但因為 test_multilora_system 是獨立的，我們直接改寫 forward 用 context 比較簡單
        # 為了不依賴 Global Context，這裡簡單用一個屬性注入 (不推薦但可行) 或是恢復 Global Context
        # 考慮到這只是測試碼，我們恢復簡易的 Global Context
        pass
...
# Helper for Context Injection (Restoring minimal context for forward pass)
class _LoRAContext:
    mapping = None
def get_mapping(): return _LoRAContext.mapping
def set_mapping(m): _LoRAContext.mapping = m

# Monkey patch forward to use this context
def _patched_forward(self, x):
    base_out = self.base_layer(x)
    m = get_mapping()
    if m is None: return base_out
    
    x_lora = x.to(self.lora_As.dtype)
    A = self.lora_As.index_select(0, m)
    B = self.lora_Bs.index_select(0, m)
    
    if x_lora.dim() == 2:
        h = torch.einsum("bi,bir->br", x_lora, A)
        out = torch.einsum("br,bro->bo", h, B)
    else:
        h = torch.einsum("bti,bir->btr", x_lora, A)
        out = torch.einsum("btr,bro->bto", h, B)
    return base_out + (out.to(base_out.dtype) * self.scaling)

DynamicLoRALinear.forward = _patched_forward
MultiLoRAEngine._apply_mapping = lambda self, m: set_mapping(m)