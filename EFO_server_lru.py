...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] EFO(LRU): %(message)s")
logger = logging.getLogger("EFOServer_LRU")
...
# LRU 狀態追蹤器 (OrderedDict 維護 MRU 順序)
cluster_lru_caches: Dict[str, OrderedDict] = {}
...
# ============================================================
def init_cluster_lru(cluster_name: str):
    """初始化 Cluster 的 LRU Cache，強制放入 Local LoRAs (不可驅逐)"""
    if cluster_name not in cluster_lru_caches:
        cluster_lru_caches[cluster_name] = OrderedDict()
        for lora_id, info in global_lora_metadata.items():
            if info.get("type") == "local" and info.get("cluster") == cluster_name:
                cluster_lru_caches[cluster_name][lora_id] = True
...
async def run_efo_metrics_cycle(step_id: int):
    """對應 Time Edge，定期拉取 Control Node 數據並寫入 log"""
                ...
                for cluster_name, url in active_clusters.items():
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            global_snapshot["efo_totals"]["total_offloads"] += data.get("offload_out", 0) # LRU 通常為 0
...
app = FastAPI(title="Edge Federation Orchestrator (LRU Baseline)", lifespan=lifespan)

@app.post("/access_lora")
async def access_lora(req: LRURequest):
    """Control Node 發生 Cache Hit 時，將目標 LoRA 推到 LRU 的 MRU 端"""
    cache = cluster_lru_caches.get(req.cluster_name)
    if cache is not None and req.lora_id in cache:
        cache.move_to_end(req.lora_id)
    return {"status": "ok"}

@app.post("/fetch_and_evict_lora")
async def fetch_and_evict_lora(req: LRURequest):
    """Control Node 發生 Cache Miss 時，模擬下載並執行 LRU 驅逐"""
    cluster = req.cluster_name
    lora_id = req.lora_id
    
    cache = cluster_lru_caches.get(cluster)
    if cache is None:
    ...
    logger.info(f"💾 [LRU] {cluster} fetched {lora_id} | Evicted: {evicted} | Cache Size: {len(cache)}/{CAPACITY}")
    return {"status": "ok", "evicted": evicted, "downloaded": True, "current_cache": list(cache.keys())}
...
async def register_cluster(req: RegisterClusterRequest):
    ...
    init_cluster_lru(req.cluster_name)
    initial_cache = list(cluster_lru_caches[req.cluster_name].keys())
    
    logger.info(f"🔗 Cluster Registered: {req.cluster_name}. Initial cache size: {len(initial_cache)}")
